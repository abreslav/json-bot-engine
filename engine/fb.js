...
const { logging, facebook: config, http } = require("../app-config.json")
const mailer = require('./mailgun-mailer')

module.exports.testOnly = {}

module.exports.installWebhook = function(app, path, webhookVerificationToken, engine, scheduler) {
    ...
    for (let i = 0; i < messaging_events.length; i++) {
        ...
        if (event["policy-enforcement"]) {
            ...
            let emailResult = await context().mailer.sendEmail(
                [config.policy_enforcement_email],
                "Alert: Policy enforcement message",
                JSON.stringify(event, null, 2)
            )
            if (emailResult) {
                console.error("Alert email sent to " + config.policy_enforcement_email)
            }
            continue
        }
        let operatorMessage = event.sender.id === config.page_id
...
module.exports.testOnly.handleRequest = handleRequest
...
const FBSender = function(userId) {
    ...
    this.fetchUserVariables = async () => {
        ...
        if (response && !response.body.error) {
            ...
            let result = {}
            result[PredefinedVariables.server_base_url] = http.base_url
...
module.exports.testOnly.MessageBuilder = MessageBuilder
