...
const telegram = require('../engine/telegram')(config)
...
mongoStorage.connect(
    async (storage) => {
        let tgTests = require('./test-data/tg-tests')

        let testNames = process.argv.slice(2)
        if (testNames.length === 0) {
            testNames = Object.keys(tgTests)
        }

        for (let testName of testNames) {
            let test = tgTests[testName]
            if (!test) {
            ...
            let createContext = (userId) => {
                return {
                    ...
                    userId: userId,
                    messageBuilder: telegram.testOnly.MessageBuilder,
                    mailer: {
            ...
            if (test.script) {
                for (let command of test.script) {
                    ...
                    if (command.goto) {
                        ...
                        await telegram.testOnly.handleRequest(
